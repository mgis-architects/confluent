...
firewall-cmd --zone=public --list-ports
#
docker run -d --net=host --name=schema-registry02 \
  -e SCHEMA_REGISTRY_KAFKASTORE_CONNECTION_URL=10.9.3.4:22181,10.9.3.5:32181,10.9.3.6:42181 \
  -e SCHEMA_REGISTRY_HOST_NAME=10.9.3.8 \
  -e SCHEMA_REGISTRY_LISTENERS=http://10.9.3.8:28081 \
...
docker run -d \
  ...
  -e KAFKA_REST_LISTENERS=http://10.9.3.8:28082 \
  -e KAFKA_REST_SCHEMA_REGISTRY_URL=http://10.9.3.8:28081 \
  -e KAFKA_REST_HOST_NAME=10.9.3.8 \
  confluentinc/cp-kafka-rest:3.1.2
#
docker run -d \
  --name=kafka-connect02 \
  --net=host \
  ...
  -e CONNECT_REST_ADVERTISED_HOST_NAME=10.9.3.8 \
